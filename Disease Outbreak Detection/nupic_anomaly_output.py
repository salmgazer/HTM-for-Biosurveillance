  ...
  def write(self, timestamp, value, predicted, anomalyScore):
    if timestamp is not None:
      """anomalyLikelihood = self.anomalyLikelihoodHelper.anomalyProbability(
        len(value), anomalyScore, timestamp
      )"""
      """if anomalyLikelihood >= 8.0:
        print "Possible outbreak ====   "+value+"  == anomalyScore: "+str(anomalyScore)+"  anomalyLikelihood: "+str(anomalyLikelihood)
      else:
        print "Not anomalous"""
